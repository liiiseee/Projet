...
public class AuthentificationServlet extends HttpServlet{
	...
	/**
	 * traitement de la requête envoyée au click de 'Se Déconnecter' de la page 'accueil.jsp' <br/>
	 * C'est une requête http GET avec l'url http://localhost:8080/gestion-clients/authentification-servlet?destroy=true
	 */
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*=======================================================================================*/
		/**
		 * déconnexion = destruction de l'objet HttpSession via la méthode invalidate()
		 */
		
		/*1. récup de la session associée à la requête (conseiller)*/
		/**
		 * getSession() : récup de la session
		 */
		HttpSession session = request.getSession(false); // false = si la session n'existe pas, ne la crée pas
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/**
		 * 	> getSession() : récup de la session associée à la requête
		 * 			-> true : si la session n'existe pas, on demande au serveur de la créer
		 */
		HttpSession session = request.getSession(true);
		
		/* 2. vérif que l'utilisateur est connecté==========================================*/
		/**
		 * la vérif se fait via l'attribut de session 'isLogged' définie lors de l'authentification du conseiller
		 */
